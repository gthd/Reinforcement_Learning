    ...
    def visualise(self):
        for j in range(1, 100):
            ...
            for step in range(30):
                ...
                action = self.policy.get_action(img_tensor.unsqueeze(0).permute(0,3,1,2).float().to(self.device), numerical_state_tensor.unsqueeze(0).float().to(self.device))
                ...
                reward = -0.1
                success = 0
                if new_sawyer_position[2] > self.termination_height or not self.robot.check_object_inbounds():
                    done = True
